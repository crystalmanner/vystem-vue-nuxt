<template>
  <div>
    <VerstoneGrid :data = "all_underwriting"> 
      <e-columns>
...
export default {
  props: ["all_underwriting", "all_loans"],
  ...
  methods: {
    onClick: function(args) {
      ...
      var selectedData = this.all_underwriting[rowIndex];  
      for( var i = 0; i < this.all_loans.length; i++ ){
        if(this.all_loans[i].loan_number == selectedData.loan_number){
            this.$router.push("/loans/" + this.all_loans[i].loan_id + "/underwriting");         
        }
      } 
    }
  }
